...
class Preferences {
	...
	public function setValue( $user, $app, $key, $value ) {
		// Check if the key does exist by trying to pull old value

		$old_value = $this->getValue($user, $app, $key);
		$exists = $old_value !== null;
		if( !$exists ) {
			$data = array(
				'userid' => $user,
				'appid' => $app,
			...
			// only update the key if the value has changed
			if( $old_value !== $value) {
				$data = array(
					'configvalue' => $value,
				);
